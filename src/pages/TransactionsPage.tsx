...
import { transactionsData } from '@/data/constants';
    ...
    <div ref={searchContainerRef} className="space-y-8 animate-fadeIn pb-12 font-sans min-h-screen">
      <div className="bg-card border-b border-border py-10 px-6">
        <div className="max-w-7xl mx-auto">
          <h1 className="text-3xl md:text-4xl font-bold text-foreground mb-2">Online <span className="text-primary">Transactions</span></h1>
          <p className="text-muted-foreground max-w-2xl">Access all your essential digital services and student portals in one place.</p>
        </div>
      </div>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-4">
        <SectionSearch containerRef={searchContainerRef} placeholder="Search transactions, services…" />
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {transactionsData.map((item, idx) => (
            <Link key={idx} to={`/transactions/${item.title.toLowerCase().replace(/[\s?]+/g, '-').replace(/-+$/, '')}`} data-searchable className={`group ${item.bg} p-5 rounded-xl border-2 ${item.color.replace('text-', 'border-').replace('600', '200').replace('700', '200')} transition-all duration-200 cursor-pointer flex items-start gap-4 hover:-translate-y-1`}
              style={{
