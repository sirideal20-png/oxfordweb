...
import { phdProgramsData } from '@/data/constants';
import {
...
/* ── Helper: find PhD programme by slug ── */

const generateSlug = (title: string) =>
  title.replace(/^Ph\.D in /, '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
...
const findPhdProgram = (slug: string) => {
  for (const [category, programs] of Object.entries(phdProgramsData)) {
...
/* ── Centralised admissions data (from Admissions menu — PhD specific) ── */
...
const centralFees = [
  { item: "Tuition Fee (Ph.D)", amount: '£8,000 / year' },
  { item: 'Programme Duration', amount: '3–4 Years (Full-Time)' },
  { item: 'Part-Time Option', amount: '6–7 Years' },
];
...
const centralScholarshipSections = [
  {
    ...
    items: [
      ...
      { name: 'Commonwealth Scholarships', detail: 'Includes Split-site PhDs. Provides full funding for students whose research contributes to national development.' },
      { name: 'Marshall Scholarships', detail: 'Highly prestigious; primarily for high-achieving students to pursue postgraduate research at any UK institution.' },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      { name: 'Alumni Loyalty Fee Waiver', detail: "A 15% to 20% discount for students progressing from a Master's to a Ph.D at the same university." },
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'Research Excellence Awards', detail: 'Awards of £2,000 to £10,000 for doctoral candidates demonstrating outstanding research potential.' },
...
const generalGuidelines = [
  ...
  'Shortlisted candidates will be invited for a discussion with faculty experts to assess research potential and suitability.',
...
const PhdProgramDetailPage = () => {
  const { slug } = useParams<{ slug: string }>();
  const navigate = useNavigate();
  const program = findPhdProgram(slug || '');
  ...
  if (!program) {
    return (
      <div className="text-center py-16 animate-fadeIn">
        ...
        <button onClick={() => navigate('/programs/ph.d-degree')} className="text-primary font-semibold text-sm hover:underline">
          ← Back to Ph.D Programmes
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-fadeIn max-w-4xl">
      ...
      <div className="flex items-center gap-2 text-xs text-muted-foreground">
        <button onClick={() => navigate('/programs/ph.d-degree')} className="flex items-center gap-1 hover:text-foreground transition-colors">
          <ArrowLeft size={14} /> Ph.D Programmes
        </button>
      ...
      <div className="bg-gradient-to-br from-primary/10 via-primary/5 to-transparent border border-primary/20 rounded-2xl p-6 md:p-8">
        <div className="flex flex-wrap gap-2 mb-3">
          ...
          <span className="text-[10px] font-semibold text-muted-foreground bg-muted px-2 py-0.5 rounded">Postgraduate Research</span>
        </div>
        <h1 className="text-2xl md:text-3xl font-bold text-foreground mb-4">{program.title}</h1>
        <div className="flex flex-wrap gap-4 text-xs text-muted-foreground">
          <span className="flex items-center gap-1.5"><Clock size={14} className="text-primary" /> 3–4 Years (Full-Time)</span>
          <span className="flex items-center gap-1.5"><GraduationCap size={14} className="text-primary" /> Postgraduate Research</span>
          <span className="flex items-center gap-1.5"><PoundSterling size={14} className="text-primary" /> £8,000/year</span>
          <Link
            to={`/schools/${program.schoolId}`}
            state={{ fromProgram: { path: `/programs/ph.d-degree/${slug}`, title: program.title } }}
      ...
      <Section icon={BookOpen} title="Programme Introduction" color="text-blue-600">
        <p className="text-sm text-muted-foreground leading-relaxed">
          The {program.title} at Oxford Skills Center of Technology is a doctoral research programme designed to produce original contributions to knowledge in the field. Candidates undertake independent, supervised research over 3–4 years (full-time) or 6–7 years (part-time), culminating in a thesis that is examined by leading academics. This programme develops world-class researchers, academics, and thought leaders within the {program.schoolName}.
        </p>
      </Section>
      ...
      <Section icon={Briefcase} title="Scope & Job Opportunities" color="text-emerald-600">
        <ul className="grid sm:grid-cols-2 gap-2">
          {[
            'University lecturer or professor positions',
            'Senior research scientist in industry or academia',
            'Director of research at think tanks and institutes',
            'Policy advisor to government and international bodies',
            'Independent consultant and expert witness',
            'Post-doctoral research fellowships worldwide',
          ].map((item, i) => (
      ...
      <Section icon={Users} title="Research & Collaboration Opportunities" color="text-amber-600">
        <ul className="space-y-2">
          {[
            'Funded research assistantship positions within university centres of excellence',
            'International conference attendance and paper presentation opportunities',
            'Collaborative research projects with industry and government partners',
            'Teaching assistantship roles to develop academic pedagogy skills',
            'Access to inter-institutional research networks and visiting scholar programmes',
          ].map((item, i) => (
            <li key={i} className="flex items-start gap-2 text-sm text-muted-foreground">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <Section icon={Award} title="Programme Benefits" color="text-rose-600">
          <ul className="space-y-2">
            {[
              'Original contribution to knowledge in your field',
              'One-on-one supervision from world-class academics',
              'Access to advanced research labs and library resources',
              'Professional development and transferable skills training',
        ...
        <Section icon={MapPin} title="Settlement Options in UK" color="text-sky-600">
          <ul className="space-y-2">
            {[
              'Graduate Route visa allows 3 years post-study work for doctoral graduates',
              'Global Talent visa route for exceptional researchers',
              'Strong demand for PhD holders in UK universities and R&D sectors',
              'Average salary £35,000–£55,000+ for post-doctoral and senior research roles',
              'Clear pathway to Indefinite Leave to Remain (ILR) through skilled work',
            ].map((item, i) => (
              <li key={i} className="flex items-start gap-2 text-sm text-muted-foreground">
      ...
      <Section icon={DollarSign} title="Fees & Scholarships" color="text-emerald-600">
        ...
        <div className="bg-amber-50 p-4 rounded-lg border border-amber-100 flex gap-3 items-start mb-6">
          ...
          <div>
            <h4 className="font-bold text-foreground text-sm">Research Scholarships Available</h4>
            <p className="text-xs text-muted-foreground mt-1">Fully funded doctoral scholarships covering tuition and living stipends are available for exceptional research candidates. Apply during your admission process.</p>
          </div>
        </div>
      ...
      <Section icon={FileCheck} title="Entry Requirements" color="text-indigo-600">
        <p className="text-sm text-muted-foreground mb-4">
          The following documents are required for Ph.D (Postgraduate Research) applicants:
        </p>
        <ul className="space-y-3">
          {phdEntryDocs.map((doc, i) => (
            <li key={i} className="flex items-start gap-2 text-sm text-muted-foreground">
      ...
      <div className="bg-card border border-border rounded-2xl p-6 md:p-8 space-y-4">
        <h3 className="text-lg font-bold text-foreground">Ready to Pursue Doctoral Research?</h3>
        <p className="text-sm text-muted-foreground">
          Download the full prospectus for detailed research areas and supervision information, or apply directly to secure your place for September 2026.
        </p>
...
export default PhdProgramDetailPage;
