...
import { diplomaProgramsData } from '@/data/diplomaPrograms';
import {
...
/* ── Helper: find Diploma programme by slug ── */

const generateSlug = (title: string) =>
  title.replace(/^Diploma in /, '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
...
const schoolMapping: Record<string, { schoolId: string; schoolName: string; schoolBrief: string }> = {
  ...
  'Economics & Development': {
    schoolId: 'bus', schoolName: 'School of Business',
    schoolBrief: 'Economics programmes are offered through the School of Business, combining economic theory with practical business analytics.',
  },
  ...
  'Library & Information Science': {
    schoolId: 'cs', schoolName: 'School of Computing & AI',
    schoolBrief: 'Library and Information Science programmes are offered through the School of Computing & AI, combining information management with digital technologies.',
  },
  'Defence & Strategic Studies': {
    schoolId: 'law', schoolName: 'School of Law & Policy',
    schoolBrief: 'Defence and Strategic Studies programmes are hosted within the School of Law & Policy, providing training in security analysis and geopolitics.',
  },
  'Public Health & Epidemiology': {
    schoolId: 'health', schoolName: 'School of Health Sciences',
    schoolBrief: 'Public Health programmes are housed within the School of Health Sciences, providing training in epidemiological methods and health policy.',
  },
  'Veterinary Sciences': {
    schoolId: 'agri', schoolName: 'School of Agriculture',
    schoolBrief: 'Veterinary Sciences programmes are hosted within the School of Agriculture, with clinical facilities and animal research labs.',
  },
  ...
  'Pharmacy & Pharmaceutical Sciences': {
    schoolId: 'health', schoolName: 'School of Health Sciences',
    schoolBrief: 'Pharmacy programmes are housed within the School of Health Sciences, with pharmaceutical labs and clinical dispensaries.',
  },
...
const findDiplomaProgram = (slug: string) => {
  for (const [category, programs] of Object.entries(diplomaProgramsData)) {
...
/* ── Centralised admissions data (Diploma specific) ── */

const diplomaEntryDocs = [
  ...
  { label: 'Personal Statement', detail: 'A brief statement (300–500 words) outlining your interest in the programme and career goals.' },
  { label: 'English Language Proficiency', detail: 'IELTS 5.5 or equivalent for international applicants whose first language is not English.' },
...
const getCentralFees = (duration: string) => [
  { item: "Tuition Fee (Diploma)", amount: '£6,000 (Total)' },
...
const centralScholarshipSections = [
  ...
  },
  {
    ...
    items: [
      { name: 'Early Payment Discount (EPD)', detail: 'A reduction of £500 to £1,000 for students who settle their full annual tuition fee by the specified deadline.' },
  ...
  },
  {
    ...
    items: [
      { name: 'Alumni Loyalty Fee Waiver', detail: 'A 10% to 15% discount for students progressing from a certificate to a diploma at the same institution.' },
...
const DiplomaProgramDetailPage = () => {
  const { slug } = useParams<{ slug: string }>();
  const navigate = useNavigate();
  const program = findDiplomaProgram(slug || '');
  ...
  if (!program) {
    return (
      <div className="text-center py-16 animate-fadeIn">
        ...
        <button onClick={() => navigate('/programs/diploma-programs')} className="text-primary font-semibold text-sm hover:underline">
          ← Back to Diploma Programmes
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-fadeIn max-w-4xl">
      ...
      <div className="flex items-center gap-2 text-xs text-muted-foreground">
        <button onClick={() => navigate('/programs/diploma-programs')} className="flex items-center gap-1 hover:text-foreground transition-colors">
          <ArrowLeft size={14} /> Diploma Programmes
        </button>
      ...
      <div className="bg-gradient-to-br from-primary/10 via-primary/5 to-transparent border border-primary/20 rounded-2xl p-6 md:p-8">
        <div className="flex flex-wrap gap-2 mb-3">
          ...
          <span className="text-[10px] font-semibold text-muted-foreground bg-muted px-2 py-0.5 rounded">Diploma</span>
        </div>
        <h1 className="text-2xl md:text-3xl font-bold text-foreground mb-4">{program.title}</h1>
        <div className="flex flex-wrap gap-4 text-xs text-muted-foreground">
          <span className="flex items-center gap-1.5"><Clock size={14} className="text-primary" /> {program.duration || '12 Months'} (Full-Time)</span>
          <span className="flex items-center gap-1.5"><GraduationCap size={14} className="text-primary" /> Diploma</span>
          ...
          <Link
            ...
            state={{ fromProgram: { path: `/programs/diploma-programs/${slug}`, title: program.title } }}
      ...
      <Section icon={PoundSterling} title="Tuition Fees & Structure" color="text-emerald-500">
        <div className="grid gap-2">
          {getCentralFees(program.duration || '12 Months').map((fee, i) => (
      ...
      <Section icon={Briefcase} title="Scope & Job Opportunities" color="text-indigo-500">
        <div className="grid gap-2">
          {[
            'Gain practical skills and industry-recognized qualifications for immediate career entry.',
            'Graduates are equipped for technician, assistant, and supervisory roles in their field.',
            'Pathway to further study — diploma holders can progress to bachelor degree programmes.',
      ...
      <Section icon={Sparkles} title="Internship & Placement Offers" color="text-pink-500">
        <div className="grid gap-2">
          {[
            'Mandatory work placement in Year 2 with partner organisations across the UK.',
      ...
      <Section icon={Globe} title="UK Settlement Options" color="text-sky-500">
        <div className="grid gap-2">
          {[
            ...
            'Student Visa Extensions — Options to extend stay for further study towards a bachelor or master degree.',
          ].map((item, i) => (
            <div key={i} className="flex items-start gap-2">
...
export default DiplomaProgramDetailPage;
